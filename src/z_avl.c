...
Z_Avl_Node *z__avl_new(Z_Allocator *allocator, void *key, void *value)
{
  Z_Avl_Node *n = z_allocator_malloc(allocator, sizeof(Z_Avl_Node));
  n->key = key;
  n->value = value;
  n->height = 1;
  n->left = NULL;
...
{
  if (*root == NULL) {
    *root = z__avl_new(allocator, key, value);
  ...
  Z_Key_Value old_key_value = z__avl_put(allocator, next_node, key, value, compare_keys);
...
)
{
  if (*root == NULL) {
    return (Z_Key_Value){ .key = NULL, .value = NULL };
     ...
     return z__avl_remove(allocator, &(*root)->right, key, compare_keys);
  } else if (compare_keys(key, (*root)->key) < 0) {
    return z__avl_remove(allocator, &(*root)->left, key, compare_keys);
  }
  ...
  if ((*root)->left == NULL || (*root)->right == NULL) {
    ...
    z_allocator_free_pointer(allocator, *root);
    *root = tmp;
    return old_key_value;
  }
  ...
  z__avl_remove(allocator, &(*root)->right, succesor->key, compare_keys);
  z__avl_rebalance_node(root);
  return old_key_value;
}
...
)
{
  ...
  z__avl_to_array_implementation(allocator, root->left, output_array);
  ...
  z__avl_to_array_implementation(allocator, root->right, output_array);
}
...
{
  Z_Key_Value_Array array = z_array_new(allocator, Z_Key_Value_Array);
  z__avl_to_array_implementation(allocator, root, &array);
  return array;
}
...
)
{
  ...
  Z_Key_Value_Array pairs = z__avl_to_array(&allocator, root);
